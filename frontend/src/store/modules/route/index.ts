...
import {fetchWorldLanguage} from '@/service/api/common';
import {getIMToken} from '@/store/modules/auth/shared';
import {localStg} from '@/utils/storage';
import {openIMLogin} from '@/utils/openim';
...
export const useRouteStore = defineStore(SetupStoreId.Route, () => {
  ...
  async function initAuthRoute() {
    ...
    // 初始化操作
    // 获取全球语言
    // await fetchWorldLanguage()
    //   .then(res => {
    //     localStg.set('worldLanguage', res.data?.list);
    //   })
    //   .catch(err => {
    //     console.error(err);
    //
    ...
    // 连接IM
    // const imToken2: null | Api.Auth.IMToken = getIMToken();
    // if (imToken2) {
    //   openIMLogin(imToken2);
    // } else {
    //   // IM 登录失败
    //   window.$notification?.error({
    //     content: 'socket连接失败',
