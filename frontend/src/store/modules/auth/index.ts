...
export const useAuthStore = defineStore(SetupStoreId.Auth, () => {
  ...
  const imToken: Api.Auth.IMToken = reactive({
    token: '',
    expireTimeSeconds: 0,
    user_id: ''
  });
  ...
  async function login(userName: string, password: string, rememberMe = false, redirect = true) {
    ...
    if (!error) {
      const pass = await loginByToken(loginResponse);
      ...
      if (pass) {
        ...
        window.$notification?.success({
          ...
          content: $t('page.login.common.welcomeBack', { userName: userInfo.nick_name }),
          duration: 4500
        });
  ...
  async function loginByToken(loginToken: Api.Auth.LoginToken) {
    ...
    localStg.set('expires_in', Date.now() + loginToken.expires_in * 1000 - 10000);

    // 2. get user info
    // const pass2 = await imLogin();
    const pass1 = await getUserInfo();
  ...
  function setUserInfo(newUserInfo: Partial<Api.Auth.UserInfo>) {
